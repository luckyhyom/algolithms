...
class Queue {
 ...
 public:
   ...
   * Node가 자신의 앞, 뒤 Node를 알고 있다면 Queue가 모든 Node를 알고 있지
   * 않아도 된다.
   *
   * Note:
...
class Graph {
  class Node {
   public:
    ...
    Node(int data) : data(data) { adjacent = LinkedList<Node*>(); };
  };
 ...
 public:
  ...
  void addEdge(int a, int b) {
    ...
    if (nodes.getValue() != nullptr) {
      node =
          *(nodes.getValue());  // subtask 1) head Node를 조회할 수 있도록
                                // 개선할 것? 애초에 데이터만 조회가 가능한가..
    } else {
      find_a = new Node(a);
      nodes.add(find_a);
    }
    ...
    while (node != nullptr) {
      ...
      if (nodes.next()->getValue() != nullptr) {
        node = *(nodes.getValue());  // subtask) nullptr이므로 접근하면 오류
                                     // 발생, 더 나은 코드는?
      } else {
        node = nullptr;
      }
  ...
  void bfs(int a = 0) {
    ...
    // subtask) Graph의 Node와 Linked List의 Node를 구분하기 어렵다.
    // 아 도대체 next하면 컨테이너를 반환해야하는거야 노드를 반환해야하는거야..?
    // LinkedList의 getValue가 LinkedList의 Node를 반환해야하는거 아닌가..?

    Node* startNode = *(nodes.getValue());
    while (startNode != nullptr) {
      if (startNode->data == a) {
        break;
      }
      if (startNode->next()->getValue() != nullptr) {
        startNode = *(nodes.getValue());
      } else {
        startNode = nullptr;
      }
    }
